...
def metro_breakpoints(source, destination, line):
    ...
    start = current_metro_line.find(source)
    end = current_metro_line.find(destination)
    ...
    if break_points == 0:
        ...
        indices = [(intermediate_stations//4) + 1, (3*(intermediate_stations)//4) + 1]
    else:
        indices = [((intermediate_stations//6) + 1), (intermediate_stations//2) + 1, (5*(intermediate_stations)//6) + 1]
...
coor1 = geolocator(loc1)
coor2 = geolocator(loc2)
lat1 = coor1[0]
lng1 = coor1[1]
lat2 = coor2[0]
lng2 = coor2[1]
...
Finaldistance.append(routes[0].get('legs')[0].get('distance').get('value'))
Finaltimes.append(routes[0].get('legs')[0].get('duration').get('value'))
aa=[]
